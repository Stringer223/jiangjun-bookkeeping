...
def hook() -> subprocess.CompletedProcess:
    ...
    但保持一致更省心）。
    """
    return sh("sh", ".githooks/pre-commit")
...
def unborn_head_case() -> None:
    ...
    check("T-N2 unborn: 钩子放行", 0, r("sh", ".githooks/pre-commit"))
...
def _run_cases() -> None:
    ...
    # ==================== 钩子层测试 ====================
    # 钩子是纯 sh，可以手动执行预览判决、不必真提交 —— 整个验证体系里最省事的一环。
    ...
    # unborn HEAD 的边界情形，用独立临时仓库跑
    unborn_head_case()

    # 非 ASCII 文件名的情形（git 的 core.quotePath 会静默踢掉这类文件）
    non_ascii_path_case()

    # 纯文档变更 -> 不该要求质量检查（T-E 那条断言的反面）
    ...
    proc = subprocess.run(["sh", ".githooks/pre-commit"], cwd=str(other), capture_output=True)
    results.append((
